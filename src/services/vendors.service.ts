...
import { vendors, vendorProductGroups, productGroups } from '../db/schema';
...
export async function getVendors() {
  ...
  return rows.map((v) => ({ ...v, productGroupIds: pgMap.get(v.id) ?? [] }));
}
...
export async function getVendorById(id: string) {
  ...
  return { ...vendor, productGroups: links };
}
...
export async function createVendor(input: CreateVendorInput, caller: AccessTokenPayload) {
  const [created] = await db
    ...
    .returning();
  return { ...created, productGroupIds: [] };
}
